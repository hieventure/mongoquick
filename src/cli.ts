...
async function handleProfileCommands(): Promise<void> {
  const profileCmd = new ProfileCommands();
  ...
  switch (subCommand) {
    case 'add':
      ...
      if (nameIndex === -1 || uriIndex === -1) {
        ...
        console.log(
          'Usage: mongoquick profile add --name <name> --uri <uri> [--database <db>] [--env <env>]'
        );
        process.exit(1);
      }
...
function showHelp(): void {
  ...
  console.log(
    '  mongoquick profile add --name prod --uri mongodb+srv://cluster.mongodb.net --env production'
  );
